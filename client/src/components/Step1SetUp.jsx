...
function Step1SetUp({ onStart }) {
    ...
    const handleUploadResume = async () => {
        ...
        try {
            ...
            setRole(prevRole => prevRole && prevRole.trim() !== "" ? prevRole : (result.data.role || ""));
            setExperience(prevExp => prevExp && prevExp.trim() !== "" ? prevExp : (result.data.experience || ""));
    ...
    const handleStart = async () => {
        if (userData && userData.credits < 50) {
            setSetupError(`Not enough credits. Minimum 50 credits required to start an interview (Your balance: ${userData.credits || 0}).`);
            return;
        }
        setSetupError("");
        setLoading(true)
